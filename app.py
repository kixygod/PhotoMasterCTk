...
class ImageProcessor:
    ...
    def discard_changes(self):
        ...
        image_brightness = enhancer.enhance(1.0)
        enhancer = ImageEnhance.Contrast(image_brightness)
        image_contrast = enhancer.enhance(1.0)
        enhancer = ImageEnhance.Color(image_contrast)
        self.image_final = enhancer.enhance(1.0)

    def changes_new(self, value1, value2, value3):
        brightness = value1
        contrast = value2
        saturation = value3
        enhancer = ImageEnhance.Brightness(self.original_image)
        ...
        enhancer = ImageEnhance.Color(image_contrast)
        self.image_final = enhancer.enhance(saturation)

    def get_final(self):
        return self.image_final

    def save_image(self, file_path):
        self.image_final.save(file_path)
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.brightness_slider = self.slider_factory.create_slider(
            self.tool_frame, "Яркость", self.get_brightness, 1
        )
        self.contrast_slider = self.slider_factory.create_slider(
            self.tool_frame, "Контрастность", self.get_contrast, 3
        )
        self.saturation_slider = self.slider_factory.create_slider(
            self.tool_frame, "Насыщенность", self.get_saturation, 5
        )
    ...
    def update_image(self):
        self.newnew_image = self.image_processor.get_final()
        self.photo.configure(
            light_image=self.newnew_image, dark_image=self.newnew_image
        )

    def get_brightness(self, value):
        self.brightness_value = float(value)
        self.apply_changes()

    def get_contrast(self, value):
        self.contrast_value = float(value)
        self.apply_changes()

    def get_saturation(self, value):
        self.saturation_value = float(value)
        self.apply_changes()

    def apply_changes(self):
        self.image_processor.changes_new(
    ...
    def discard_changes(self):
        self.image_processor.discard_changes()
        self.update_image()
